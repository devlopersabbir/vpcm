    ...
    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-30 flex justify-end">
      <div className="w-full max-w-3xl bg-slate-900/95 border-l border-slate-850 h-full flex flex-col shadow-2xl relative">
        ...
        <button
          ...
          className="absolute top-6 right-6 p-2 bg-slate-850 hover:bg-slate-750 text-slate-400 hover:text-slate-200 rounded-full transition-colors z-10"
        >
          <X className="h-4 w-4" />
        </button>
            ...
            <div className="space-y-6">
              <div className="bg-slate-950/50 border border-slate-850 rounded-2xl p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div>
                    <span className="text-slate-550 block uppercase font-bold tracking-wider text-[9px] mb-0.5">UUID</span>
                    <span className="font-mono text-slate-300 font-semibold">{selectedServer.uuid}</span>
                  </div>
                  <div>
                    <span className="text-slate-550 block uppercase font-bold tracking-wider text-[9px] mb-0.5">Credential Method</span>
                  ...
                  <div>
                    ...
                    <span className="text-slate-355 font-semibold">{new Date(selectedServer.created_at).toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-slate-550 block uppercase font-bold tracking-wider text-[9px] mb-0.5">Last Audited</span>
                    <span className="text-slate-355 font-semibold">
                    ...
                    <tbody className="divide-y divide-slate-850/40">
                      {logs.map((log: any) => (
                        <tr key={log.id} className="hover:bg-slate-900/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-slate-450">{new Date(log.logged_in_at).toLocaleString()}</td>
